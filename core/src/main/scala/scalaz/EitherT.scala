...
sealed trait EitherT[F[+_], +A, +B] {
  ...
  def ap[C, AA >: A](f: => EitherT[F, AA, B => C])(implicit F: Apply[F]): EitherT[F, AA, C] = {
    import std.either._

    EitherT.eitherT[F, AA, C](F.map2(f.run, run)((ff: Either[AA, B => C], aa: Either[AA, B]) => eitherMonad[AA].ap(aa)(ff)))
  }
...
object EitherT extends EitherTFunctions with EitherTInstances {
  ...
  sealed trait LeftProjectionT[F[+_], +A, +B] {
    ...
    def orElse[AA >: A, BB >: B](x: => EitherT[F, AA, BB])(implicit m: Bind[F]): EitherT[F, AA, BB] = {
      val g = eitherT.run
      EitherT(m.bind(g){
        case Left(_) => g
        case Right(_) => x.run
    ...
    def flatMap[BB >: B, C](f: A => EitherT[F, C, BB])(implicit F: Monad[F]): EitherT[F, C, BB] =
