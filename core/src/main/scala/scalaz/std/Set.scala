...
trait SetInstances {
  implicit val setInstance = new Foldable[Set] with Each[Set] with Length[Set] {
    def each[A](fa: Set[A])(f: (A) => Unit) = fa foreach f
    def length[A](fa: Set[A]) = fa.size

    def foldMap[A, B](fa: Set[A])(f: A => B)(implicit F: Monoid[B]) =
      fa.foldLeft(F.zero)((b, a) => F.append(f(a), b))
